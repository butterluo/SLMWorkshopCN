...
def image_complexity(img, laplacian_var_thres=500, edge_count_thres=10000, total_entropy_thres=5.0):
    """图片的熵，清晰度，边缘数都超过阈值，则定义为复杂的，信息量大的，有意义的图片，后续需要处理"""
    if isinstance(img, Image.Image):
        img = cv2.cvtColor(np.array(img), cv2.COLOR_RGB2BGR)
    # 计算图形熵，越大图片包含的信息越多，就越复杂 https://blog.51cto.com/u_16213458/12193453
    ##### Histogram entropy 
    hist_b = cv2.calcHist([img], [0], None, [256], [0, 256])# 图片直方图 https://blog.csdn.net/hai411741962/article/details/132235279 
    ...
    # 评估图片清晰程度，Laplacian variance 越大越清晰 https://pyimagesearch.com/2015/09/07/blur-detection-with-opencv/
    ### Laplacian variance
    gray_img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    laplacian_var = cv2.Laplacian(gray_img, cv2.CV_64F).var()
    # 图片边缘检测，边缘越多图片越复杂 https://blog.csdn.net/ai_faker/article/details/114300499 
